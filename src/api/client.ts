...
export class YouGileClient {
  ...
  private async request<T>(method: string, endpoint: string, body?: Record<string, unknown>): Promise<T> {
    ...
    try {
      ...
      throw new Error(`YouGile API error (HTTP ${status || 'unknown'}): ${message}`);
    }
    if (response.status >= 400) {
  ...
  async updateTask(id: string, payload: Record<string, unknown>): Promise<Record<string, unknown>> {
    return this.request<Record<string, unknown>>('PUT', `/tasks/${encodeURIComponent(id)}`, payload);
  }
  ...
  async getMessages(chatId: string): Promise<YouGileChatMessage[]> {
    const result = await this.request<{ content: YouGileChatMessage[] }>(
      'GET',
  ...
  async uploadFile(arrayBuffer: ArrayBuffer, fileName: string): Promise<{ url: string; fullUrl: string }> {
    ...
    try {
      ...
      console.error('YouGile upload error:', errorMessage(e));
      throw e;
    }
  }
}